...
/*
Hold takes context; reference to ledger entry containing user id, idempotency
key, and amount; order id; amount of hours for hold's lifetime. It validates
request; fills in the rest of information in entry; updates user balance;
updates batches of points; holds points; inserts ledger entry. It returns id of
the new batch (0 if transaction is failed) and error (nil if transaction is
successful).
*/
