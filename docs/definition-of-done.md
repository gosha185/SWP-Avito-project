# Definition of Done

A PBI (Product Backlog Item) is considered **Done** when all of the following conditions are met:

1. **All issue-specific acceptance criteria are satisfied** – each AC has been verified and passes.
2. **The work is reviewed by another team member** – at least one other person has approved the code.
3. **The issue-linked PR/MR is merged into the protected default branch** – the code is integrated.
4. **Required tests or checks pass** – all automated tests (CI) and manual checks are successful.
5. **`CHANGELOG.md` is updated** – if the change is user-visible, it is documented.
